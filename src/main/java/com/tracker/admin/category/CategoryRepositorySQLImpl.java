...
public class CategoryRepositorySQLImpl implements CategoryRepository {
    ...
    @Override
    public List<Category> findCategoriesList(String searchText) {
        con = null;
        pstmt = null;
        rs = null;
    ...
    public boolean setNewCategory(String addCatName, String addCatDescription) {
        ...
        pstmt = null;
        category = new Category();
        category.setCategoryName(addCatName);
        category.setCategoryDescription(addCatDescription);
        try {
            con = DataBaseManager.getInstance().getConnection();
    ...
    @Override
    public Boolean updateCategoryById(int catId, String catName, String catDescription) {
        con = null;
        pstmt = null;
        try {
            con = DataBaseManager.getInstance().getConnection();
            con.setTransactionIsolation(Connection.TRANSACTION_REPEATABLE_READ);
            sqlInsertion = String.format(UPDATE_CATEGORY_BY_ID, catName, catDescription, catId);
            pstmt = con.prepareStatement(sqlInsertion);
            pstmt.executeUpdate();
            con.setTransactionIsolation(Connection.TRANSACTION_READ_UNCOMMITTED);
            return true;
